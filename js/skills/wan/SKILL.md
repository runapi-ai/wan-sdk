...
- For a one-off artifact or result, use the registered `wan` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi wan --help
runapi wan <operation> --help
curl --fail --location https://runapi.ai/docs/api/wan/<operation>.md --output contract.md
```
...
```shell
runapi wan <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service wan --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/wan-sdk
...
- `wan-2.7-edit-video`: https://runapi.ai/models/wan/2.7-edit-video.md
...
- `wan-2.7-r2v`: https://runapi.ai/models/wan/2.7-r2v.md
