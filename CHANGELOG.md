# Changelog

## [js/v0.3.4](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.3.4), [ruby/v0.2.13](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.13), [go/v0.2.14](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.14), [python/v0.2.4](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.2.4) - 2026-08-14

### Fixed
- Accept enable_safety_checker for Wan 2.7 video edit requests.


## [js/v0.3.3](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.3.3), [ruby/v0.2.12](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.12), [go/v0.2.13](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.13), [python/v0.2.3](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.2.3) - 2026-08-12

### Fixed
- Reject enable_safety_checker for Wan 2.7 video edit requests while preserving it for supported Wan models.


## [js/v0.3.2](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.3.2), [ruby/v0.2.11](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.11), [go/v0.2.12](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.12), [python/v0.2.2](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.2.2), [java/v0.1.3](https://github.com/runapi-ai/wan-sdk/releases/tag/java%2Fv0.1.3) - 2026-08-07

### Added
- Add the optional multi_shots field to WAN 2.6 text-to-video, image-to-video, and video editing requests.


## [js/v0.3.1](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.3.1), [ruby/v0.2.10](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.10), [go/v0.2.11](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.11), [python/v0.2.1](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.2.1) - 2026-07-28

### Fixed
- Validate supported duration values for Wan Flash image-to-video requests; the API applies documented duration and resolution defaults when callers omit them.


## [go/v0.2.10](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.10) - 2026-07-28

### Added
- Expose persisted billing facts on task responses.

## [js/v0.3.0](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.3.0) - 2026-07-28

### Added
- Type task billing facts on task responses.

## [ruby/v0.2.9](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.9) - 2026-07-28

### Added
- Expose live pricing through the shared core SDK.


## [python/v0.2.0](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.2.0) - 2026-07-24

### Added
- Expose shared Files, Account, and Pricing resources plus typed Task Billing Facts through the Provider Client.


## [js/v0.2.9](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.2.9), [ruby/v0.2.8](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.8), [go/v0.2.9](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.9), [python/v0.1.1](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.1.1), [java/v0.1.2](https://github.com/runapi-ai/wan-sdk/releases/tag/java%2Fv0.1.2) - 2026-07-20

### Fixed
- Reject `seed` for Wan 2.6 text-to-video and image-to-video models before sending requests.


## [js/v0.2.8](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.2.8), [go/v0.2.8](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.8) - 2026-07-17

### Fixed
- Align JavaScript and Go with the current RunAPI core parameter validation APIs.

## [java/v0.1.1](https://github.com/runapi-ai/wan-sdk/releases/tag/java%2Fv0.1.1) - 2026-06-25

### Fixed
- Fixed Java retry handling for Retry-After response headers.
- Fixed Java contract validation for action-level conditional rules.
- Refreshed Java SDK metadata for v0.1.1.

## [java/v0.1.0](https://github.com/runapi-ai/wan-sdk/releases/tag/java%2Fv0.1.0) - 2026-06-24

### Added
- Publish `ai.runapi:runapi-wan` for Java SDK consumers.
- Include typed Java builders, synchronous client resources, sources, and Javadocs.

## [js/v0.2.7](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.2.7), [ruby/v0.2.7](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.7), [go/v0.2.7](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.7), [python/v0.1.0](https://github.com/runapi-ai/wan-sdk/releases/tag/python%2Fv0.1.0) - 2026-06-18

### Changed
- Per-method documentation for all resource methods

## [js/v0.2.6](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.2.6), [ruby/v0.2.6](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.6), [go/v0.2.6](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.6) - 2026-06-01

### Changed
- Align SDK with upstream Input Contract and public API vocabulary changes
- Update endpoint definitions and field constraints

## [js/v0.2.4](https://github.com/runapi-ai/wan-sdk/releases/tag/js%2Fv0.2.4), [ruby/v0.2.4](https://github.com/runapi-ai/wan-sdk/releases/tag/ruby%2Fv0.2.4), [go/v0.2.4](https://github.com/runapi-ai/wan-sdk/releases/tag/go%2Fv0.2.4) - 2026-05-22

### Changed
- Publish JavaScript, Ruby, and Go SDK artifacts for wan with per-language GitHub release tags.
- Refresh public README metadata.

## [v0.2.1](https://github.com/runapi-ai/wan-sdk/releases/tag/v0.2.1) - 2026-05-19

Initial release.
